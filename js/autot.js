...
class AutoTracker {
  ...
  constructor(settings, textTracker) {
    this.#settings = settings;
	
	toggle_chest = id => textTracker.setLocation(id);
	setitem = (item, value) => textTracker.setItem(item, value);
  ...
  loadSettings() {
	...
	port = parseInt(settings.autoTrackingPort);
	if (port > 0 && settings.autoTracking) {
	  autotrackConnect("ws://localhost:" + port);
	}
  }
...
function autotrackConnect(host) {	
...
function autotrackDoTracking(data) {
	...
	updatechest(72, 0x226, 0x10); // King's Tomb
	...
	updatechest(69, 0x2BB, 0x40); // Sunken Treasure
